...
# Hide console window immediately (suppress return value)
$null = [Win32]::ShowWindow([Win32]::GetConsoleWindow(), 0)

# Redirect stderr to null to suppress JPEG warnings
$null = [System.Console]::SetError([System.IO.TextWriter]::Null)

# Suppress GDI+ JPEG warnings - with null check
try {
    ...
    # Ignore if we can't add the handler
}
#endregion
...
# Check admin mode (affects drag & drop)
try {
    $isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    if ($isAdmin) {
        Write-Host "Warning: Running as Administrator - Drag & Drop will not work!" -ForegroundColor Yellow
    }
} catch {
...
function Log($msg, $isError = $false) {
    ...
    try {
        if ($isError) {
            $log.AppendText("ERROR: $line`r`n")
        } else {
            $log.AppendText("$line`r`n")
        }
    } catch {
        # Fallback if log control not ready
    }
}
#endregion
...
function Add-Label($text, $x, $y) {
    $shadow = New-Object Windows.Forms.Label
    $shadow.Text = $text
    $shadow.Location = "$($x+1),$($y+1)"
    $shadow.ForeColor = [System.Drawing.Color]::Black
...
function Get-TextboxValue($textbox) {
    # Return the actual text regardless of color, but don't return placeholder text
    if ($textbox.Text -eq $textbox.Tag -and $textbox.ForeColor -eq [System.Drawing.Color]::Gray) {
        return ""
    }
    return $textbox.Text
}
...
function Update-PreviewBoxAppearance {
    ...
    try {
        $previewBox.BackColor = if ($script:previewImage) { 
...
function Update-RunState {
    ...
    if ($source -and (Test-Path $source)) {
        # Check if source has valid images
        if (Test-Path $source -PathType Container) {
            ...
            # Check if single file is a valid image
...
function Validate-PathBox($textbox) {
    ...
    # Handle placeholder/empty states
...
if (Test-Path $realText -PathType Container) {
    ...
    if (-not $isDest) {
        ...
        # For destination folder - just check if it's a valid folder (no image validation needed)
        Set-InputColor $textbox ([System.Drawing.Color]::LightGreen)
        $icon.Text = "✔"; $icon.ForeColor = "Green"
...
function Update-UnitsDisplay {
    if (-not $script:realCropRect) { 
        # No crop rect - convert the existing numeric values between units
...
function Update-SourceSizeBoxes {
    if (-not $script:previewImage) {
        # No preview image - convert the existing numeric values between units
...
function Load-PreviewImage($path) {
    ...
    try {
        ...
        if (Test-Path $path -PathType Leaf) {
            ...
            try {
                # FIXED: Use $path instead of $validFile.FullName
            ...
            $img = $null
            try {
                # FIXED: Use $validFile.FullName instead of $path
        ...
        if ($_.Exception.Message -match "Invalid SOS parameters") {
            ...
            # The image still loaded fine, so don't log it
        } else {
...
function BrowseSourceModern($targetBox) {
    ...
    $dlg.Title = "Select file(s) or folder"
    ...
    # Set initial directory to current textbox value if valid, otherwise current folder
    ...
    if ($dlg.ShowDialog($form) -eq "OK") {
        ...
        $path = if ($rawPath -match "Select folder") { Split-Path $rawPath } else { $rawPath }
        ...
        if (Test-Path $path -PathType Container) {
            ...
            # Let the validation timer handle image loading
...
function BrowseFolder($targetBox) {
    ...
    # Set initial directory priority:
    # 1. Current textbox value if valid
    # 2. Source folder if available
    # 3. Current working directory
    $currentPath = Get-TextboxValue $targetBox
    
    if (-not [string]::IsNullOrWhiteSpace($currentPath) -and (Test-Path $currentPath)) {
        # Use the current textbox path
        if (Test-Path $currentPath -PathType Container) {
        ...
        # Fallback to source folder if available
                ...
                if ($ext -match '\.(jpg|jpeg|png|bmp|gif)$') {
                    ...
                    foreach ($f in Get-ChildItem $path -File) {
                        ...
                        if ($file) {
                            ...
                            $script:previewImage = $null
                            Update-PreviewBoxAppearance
                            # Update button state since no valid images
                            Update-RunState
                        }
                }
        } else {
...
#region Form Controls - Source & Destination
# -------------------------------------------------------------------------------------------------
$form.Add_Activated({
...
$srcBox.Add_TextChanged({
    ...
    # Remove placeholder text if it's the default
    if ($path -eq "Write/paste the path or Drop a folder/file here...") {
        $path = ""
    }
    
    # Auto-update destination if it's empty or still has placeholder
    $destText = $dstBox.Text
    ...
    if ($isDestEmpty -and -not [string]::IsNullOrWhiteSpace($path)) {
        if (Test-Path $path) {
            ...
            # Only update if source folder is valid
            if (Test-Path $sourceFolder) {
                $dstBox.Text = $sourceFolder
                $dstBox.ForeColor = [System.Drawing.Color]::Black
                # Remove placeholder tag if exists
                if ($dstBox.Tag -eq "Write/paste the path or Drop the destination folder...") {
                    $dstBox.Tag = $null
                }
                Validate-PathBox $dstBox
            }
...
# Destination section
$null = Add-Label "Destination folder (if not the same as source):" 85 70
...
$dstBtn.Text = "Browse Destination"
...
$noOverwriteChk.Text = "Don't overwrite`n(add _1)"
...
$clearLogBtn.Add_Click({
    ...
    if ($croppedcountLabel) { $croppedcountLabel.Text = "" }
})
#endregion
...
$countLabel.Location = "500,210"
$countLabel.AutoSize = $true
$countLabel.TextAlign = "MiddleRight"
$countLabel.Font = New-Object System.Drawing.Font("Segoe UI", 8, [System.Drawing.FontStyle]::Bold)
$form.Controls.Add($countLabel)

$croppedCountLabel = New-Object Windows.Forms.Label
$croppedCountLabel.Location = "550,210"
$croppedCountLabel.AutoSize = $true
$croppedCountLabel.TextAlign = "MiddleCenter"
$croppedCountLabel.Font = New-Object System.Drawing.Font("Segoe UI", 14, [System.Drawing.FontStyle]::Bold)
$form.Controls.Add($croppedCountLabel)

$progress = New-Object Windows.Forms.ProgressBar
$progress.Location = "10,220"
$progress.Size = "475,20"
...
#region Preview Box with Drag & Drop and Crop Controls
# -------------------------------------------------------------------------------------------------
$previewBox = New-Object Windows.Forms.PictureBox
$previewBox.Location = "600,10"
$previewBox.Size = "260,280"
$previewBox.BorderStyle = "FixedSingle"
$previewBox.SizeMode = "Zoom"
$previewBox.AllowDrop = $true
$previewBox.Tag = $previewBox.BackColor
...
$previewBox.Add_DragDrop({
    ...
    if ($files -and $files.Count -gt 0) {
        ...
        if (Test-Path $path -PathType Container) {
            ...
            if ($firstImage) {
                ...
                [System.Windows.Forms.MessageBox]::Show(
                    "The folder contains no valid image files.`n`nSupported formats: JPG, JPEG, PNG, BMP, GIF",
                    "No Images Found",
        ...
        elseif (Test-Path $path -PathType Leaf) {
            $ext = [System.IO.Path]::GetExtension($path).ToLower()
            if ($ext -match '\.(jpg|jpeg|png|bmp|gif)$') {
                # Validate JPEG if needed
                if ($ext -match '\.jpe?g$' -and -not (Test-JpegValid $path)) {
                    [System.Windows.Forms.MessageBox]::Show(
                        "The selected JPEG file appears to be corrupted.",
                        "Invalid Image",
                        [System.Windows.Forms.MessageBoxButtons]::OK,
                        [System.Windows.Forms.MessageBoxIcon]::Warning
                    )
                ...
                [System.Windows.Forms.MessageBox]::Show(
                    "  The dropped file is not an image file.`n`n  Please drop an image file or folder containing images.`n`n  Supported formats: JPG, JPEG, PNG, BMP, GIF",
                    "Invalid File Type",
                    [System.Windows.Forms.MessageBoxButtons]::OK,
                    [System.Windows.Forms.MessageBoxIcon]::Warning
                )
...
# Paint event for crop rectangle and drag hint
...
#region Source and Destination Drag & Drop
# -------------------------------------------------------------------------------------------------
...
$cropBtn.Add_Click({
    ...
    if (-not (Test-Path $dest)) {
        try {
            ...
            Log "Created destination folder: $dest"
        } catch {
            Log "Cannot create destination folder: $dest" $true
            $cropBtn.Enabled = $true
            $cancelBtn.Enabled = $false
            return
        }
    ...
    foreach ($f in $files) {
        ...
        try {
            ...
            Log "Saved: $newName"
            $croppedCount++
            $countLabel.Text = "Images`ncropped"
            $croppedCountLabel.Text = $croppedCount
...
# Show the form and ensure clean exit
